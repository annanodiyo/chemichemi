...
Classify the report for urgency using the text description and, if provided, the attached photo.
Flag as alert-worthy if it describes or shows: dead/dying fish in numbers, visible chemical discharge,
oil sheen, unusual algae bloom, sewage discharge, or strong chemical odor.
Do NOT flag routine complaints, general questions, or vague statements with no observable incident.
If a photo is attached, describe what you actually see in it as part of your reasoning — do not assume
the photo confirms the text if it doesn't visually support it.
...
function keywordFallback(text: string): AlertClassification {
  ...
  return {
    ...
    keyPhrases: hits,
    reasoning: "Keyword fallback (AI unavailable — photo not analyzed)",
    source: "keyword_fallback",
  };
}

export const classifyReport = createServerFn({ method: "POST" })
  .validator((input: ClassifyInput) => input)
  .handler(async ({ data }): Promise<AlertClassification> => {
    const { text: reportText, photo } = data;
    ...
    try {
      ...
      const parts: Array<{ text: string } | { inlineData: { mimeType: string; data: string } }> = [
        { text: `Report text: "${reportText}"` },
      ];
      if (photo) {
        parts.push({ inlineData: { mimeType: photo.mimeType, data: photo.base64Data } });
      }
