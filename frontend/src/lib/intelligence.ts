...
export function waterQuality(
  ...
  const turbidity = clamp(
    4 + w.precipitation * 1.6 + Math.max(0, loc.riskOffset) * 8,
    0,
    120,
  );
  const nutrients = clamp(
    0.2 + w.precipitation * 0.045 + Math.max(0, loc.riskOffset) * 0.35,
    0,
    6,
  ); // mg/L total N
  const dissolvedO2 = clamp(
    9.2 -
      Math.max(0, w.temperature - 25) * 0.55 -
      Math.max(0, 5 - w.windspeed) * 0.42,
    0,
    12,
  );
  const chlorophyll = clamp(
    6 + nutrients * 9 + Math.max(0, w.temperature - 27) * 5,
    0,
    120,
  );
...
export function reportPressureFor(
  locationId: number,
...
export function buildIntel(
  reports: CommunityReport[] = getReports(),
): BeachIntel[] {
    ...
    .map((location) => {
      ...
      return {
        ...
        reports: reports.filter(
          (r) =>
            r.verificationStatus === "verified" && r.locationId === location.id,
        ).length,
      };
    })
...
export function byCounty(intel: BeachIntel[]): CountyIntel[] {
  ...
  return Array.from(map.entries())
    .map(([county, list]) => ({
      ...
      beaches: list.length,
      avgWqi: Math.round(
        list.reduce((a, i) => a + i.quality.wqi, 0) / list.length,
      ),
      worst: list.reduce((a, i) =>
        i.prediction.probability > a.prediction.probability ? i : a,
      ),
      alerts: list.filter((i) => i.level === "DANGER" || i.level === "CRITICAL")
        .length,
    }))
