...
export function assessRisk(w: WeatherData, loc: Location): RiskAssessment {
  ...
  if (w.pm2_5 !== undefined && w.pm10 !== undefined) {
    if (w.pm2_5 > 35 || w.pm10 > 100) {
      score += 2;
      factors.push({
  ...
  if (score >= 8) {
    level = "CRITICAL";
    advice =
      "CRITICAL: Move fish to surface cages or harvest immediately. Deploy aerators if available. Do not feed. Contact your BMU chairperson.";
  } else if (score >= 6) {
    level = "DANGER";
    advice =
      "DANGER: High mortality risk. Reduce cage density. Monitor every 2 hours from 4–8 AM. Prepare emergency harvest.";
  } else if (score >= 3) {
    level = "CAUTION";
    advice =
      "CAUTION: Conditions favour oxygen depletion. Watch for fish surfacing/gasping. Have aeration ready. Reduce feeding.";
  } else {
    level = "SAFE";
    advice =
      "SAFE: Conditions favourable. Maintain normal operations. Continue routine monitoring.";
  }
