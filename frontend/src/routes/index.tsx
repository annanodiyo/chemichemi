...
import { fetchEnvironmentalData, type EnvironmentalData } from "@/lib/open-meteo";
  ...
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "Chemichemi — Lake Victoria Fish Kill Early Warning" },
      {
        ...
        content:
          "Beach-level risk scores, 7-day forecasts and community pollution reports for Lake Victoria fish farmers.",
      },
...
function Dashboard() {
  ...
  useEffect(() => {
    ...
    setEnvironment(null);
    void fetchEnvironmentalData(location, controller.signal)
      .then((data) => setEnvironment(data))
      .catch((error: unknown) => {
        if (!(error instanceof DOMException && error.name === "AbortError")) {
          console.warn("Unable to load live Open-Meteo conditions; using local forecast.", error);
        }
      });
  ...
  const forecast = useMemo(
    () =>
      environment?.forecast.map((weather: WeatherData) => ({
        weather,
    ...
    <div className="mx-auto w-full max-w-6xl px-4 py-8 sm:px-6 lg:py-12">
      <header className="flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button variant="outline" size="sm" onClick={onSendSandboxAlert} disabled={sendingAlert}>
            {sendingAlert ? "Sending…" : "Send sandbox alert"}
          </Button>
      ...
      <section className="mt-10 max-w-2xl">
        ...
        <p className="mt-3 text-sm leading-relaxed text-muted-foreground sm:text-base">
          Chemichemi combines temperature, wind mixing, rainfall runoff and local beach conditions
          into one daily risk score so cage and beach seine farmers can act before a fish kill.
        </p>
      </section>
            ...
            {forecast.map(({ weather: w, assessment: a }) => (
              ...
              >
                <div className="text-xs font-semibold text-muted-foreground">
                  {w.date.toLocaleDateString("en-KE", {
                    weekday: "short",
                    month: "short",
                    day: "numeric",
                  })}
